...
predicted_data <- predicted_data + rnorm(length(predicted_data), mean = mean(predicted_data), sd = predicted_std_error)
...
```{r}
plot1 <- ggplot(predicted_data,
       mapping = aes(
         x = predicted_data,
       ))+
  geom_histogram()+
  labs(
    x = "Average GDP - Simulated",
    title = "Simulated Data: Average GDP vs Count"
  )+
  theme_bw()



plot2 <- ggplot(averaged_data,
       mapping = aes(
         x = avg_gdp,
       ))+
  geom_histogram()+
  labs(
    x = "Average GDP - Observed",
    title = "Observed Data: Average GDP vs Count"
  )+
  theme_bw()
...
- Discussion of similarities and differences between observed and simulated -- not writing rn because im not sure if i did it right :(
